...
public String addRest(Agent rest) throws ClassNotFoundException, SQLException {
con = ConnectionHelper.getConnection();
...
public String updateRest(Agent rest) throws ClassNotFoundException, SQLException {
Agent restFound = searchAgent(rest.getAgentid());
if (restFound!=null) {
String cmd = "Update Restaurant set Name=?, City=?,GENDER=?,Maritalstatus=?,"+ "Premium=? Where AgentId=? ";
pst = con.prepareStatement(cmd);
pst.setString(1, rest.getName());
pst.setString(2, rest.getCity());
pst.setString(3,rest.getGender().toString());
